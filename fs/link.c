...
PUBLIC int do_rmdir()
{
	...
	if (strcmp(pathname , "/") == 0) {
		printl("{FS} FS:do_rmdir():: cannot unlink the root(up) dir\n");
		return -1;
	}
	...
	if (pin->i_mode != I_DIRECTORY) { /* can only remove regular files */
		printl("{FS} cannot remove dir %s, because "
		       "it is not a dir file.\n",
		       pathname);
		return -1;
	}

	if (pin->i_cnt > 1) {	/* the dir was opened */
		printl("{FS} cannot remove dir %s, because pin->i_cnt is %d.\n",
		       pathname, pin->i_cnt);
		return -1;
	}

	int dir_ent_nr = pin->i_size/DIR_ENTRY_SIZE;
	if(dir_ent_nr > 1){	/*the dir was not empty*/
		printl("{FS} cannot remove dir %s, because the dir is not empty: %d.\n",
