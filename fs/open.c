...
PRIVATE struct inode * new_inode(int dev, int inode_nr, int start_sect, int flag);
PRIVATE void new_dir_entry(struct inode * dir_inode, int inode_nr, char * filename, int flag);
...
PRIVATE struct inode * create_file(char * path, int flags)
{
	...
	new_dir_entry(dir_inode, newino->i_num, filename, 1);
 ...
 *****************************************************************************/
/**
 * Create a dir and return it's inode ptr.
 *
 * @param[in] path   The full path of the new dir
 *
 * @return           Ptr to i-node of the new dir if successful, otherwise 0.
 * 
 * @see open()
 * @see do_open()
 *****************************************************************************/
PRIVATE struct inode * create_dir(char * path)
{
	...
	int inode_nr = alloc_imap_bit(dir_inode->i_dev);
	int free_sect_nr = alloc_smap_bit(dir_inode->i_dev, NR_DEFAULT_FILE_SECTS);	/*dir file */
	struct inode *newino = new_inode(dir_inode->i_dev, inode_nr,
					 free_sect_nr, 0);

	new_dir_entry(dir_inode, newino->i_num, filename, 0);
 ...
 * @param flag Type of the i_mode,0 = I_DIRECTORY, 1 = I_REGULAR
 * 
...
PRIVATE struct inode * new_inode(int dev, int inode_nr, int start_sect,int flag)
{
	...
	new_inode->i_size = 0;
	new_inode->i_parent = current_dir;
...
PRIVATE void new_dir_entry(struct inode *dir_inode,int inode_nr,char *filename, int flag)
{
	...
	if(flag){
		new_de->i_mode = I_REGULAR;
	}
	else{
		new_de->i_mode = I_DIRECTORY;
	}
